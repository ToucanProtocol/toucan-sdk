...
describe("Testing Toucan-SDK", function () {
  ...
  before(async () => {
    ...
    scoredTCO2sNCT = await toucan.getScoredTCO2s("NCT", addr1);
    scoredTCO2sBCT = await toucan.getScoredTCO2s("BCT", addr1);
  });

  describe("Testing OffsetHelper related methods", function () {
    it("Should retire 1 TCO2 using pool token deposit", async function () {
      await expect(
        toucan.autoOffsetUsingPoolToken("NCT", parseEther("1.0"), addr1)
      ).to.not.be.reverted;
    });
    ...
    it("Should retire 1 TCO2 using swap token", async function () {
      ...
      await expect(
        toucan.autoOffsetUsingSwapToken("NCT", parseEther("1.0"), weth, addr1)
      ).to.not.be.reverted;
    });

    it("Should retire 1 TCO2 using ETH deposit", async function () {
      await expect(toucan.autoOffsetUsingETH("NCT", parseEther("1.0"), addr1))
        .to.not.be.reverted;
    });
  });
  ...
  describe("Testing NCT related methods", function () {
    it("Should automatically redeem NCT", async function () {
      ...
      await expect(toucan.redeemAuto("NCT", parseEther("1.0"), addr1)).to.not.be
        .reverted;
    ...
    it("Should automatically redeem NCT & return a correct array of TCO2s", async function () {
      ...
      const tco2s = await toucan.redeemAuto2("NCT", parseEther("1.0"), addr1);
    ...
    it("Should selectively redeem NCT for the highest quality TCO2s", async function () {
      ...
      await expect(
        toucan.redeemMany("NCT", [tco2Address], [parseEther("1.0")], addr1)
      ).to.not.be.reverted;
    ...
    it("Should automatically redeem NCT & deposit the TCO2 back", async function () {
      ...
      await toucan.redeemAuto("NCT", parseEther("1.0"), addr1);

      await expect(
        toucan.depositTCO2("NCT", parseEther("1.0"), tco2.address, addr1)
      ).to.not.be.reverted;
  ...
  describe("Testing BCT related methods", function () {
    it("Should selectively redeem BCT for the highest quality TCO2s", async function () {
      ...
      const remainingTCO2 = await toucan
        .getPoolContract("BCT", addr1)
        // @ts-ignore tokenBalances is an autogenerated getter for a mapping within inherited storage
        .tokenBalances(tco2Address);

      await expect(
        toucan.redeemMany("BCT", [tco2Address], [parseEther("1.0")], addr1)
      ).to.not.be.reverted;
    ...
    it("Should automatically redeem BCT & deposit the TCO2 back", async function () {
      const TCO2 = toucan.getTCO2Contract(scoredTCO2sBCT[0], addr1);
      const tco2BalanceBefore = await TCO2.balanceOf(addr1.address);
      const bct = toucan.getPoolContract("BCT", addr1);
      const bctBalanceBefore = await bct.balanceOf(addr1.address);

      await toucan.redeemAuto("BCT", parseEther("1.0"), addr1);

      await expect(
        toucan.depositTCO2("BCT", parseEther("1.0"), TCO2.address, addr1)
      ).to.not.be.reverted;
  ...
  describe("Testing Contract Registry related methods", function () {
    it("Should return true", async function () {
      expect(await toucan.checkIfTCO2(scoredTCO2sBCT[0], addr1)).to.be.eql(
        true
      );
    });

    it("Should return false", async function () {
      expect(await toucan.checkIfTCO2(addr1.address, addr1)).to.be.eql(false);
    });
  });
  ...
  describe("Testing TCO related methods", function () {
    ...
    it("Should retire 1 TCO2 from another address", async function () {
      ...
      const TCO2 = toucan.getTCO2Contract(tco2s[0].address, addr1);
      await TCO2.approve(addr2.address, parseEther("1.0"));

      await expect(
        toucan.retireFrom(parseEther("1.0"), addr1.address, TCO2.address, addr2)
      ).to.not.be.reverted;
    });
  });
